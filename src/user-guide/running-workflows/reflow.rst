...
Flow merging is necessary because every :term:`active task` must have
a unique task ID. However, merging is also useful: it allows a single flow to
continue downstream of multi-flow interventions.
